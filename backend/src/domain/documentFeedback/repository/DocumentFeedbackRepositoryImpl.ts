...
import {ADOPTION_FEEDBACK_PATH, ADOPTION_PATH, ANIMAL_PATH, DOCUMENT_FEEDBACK_PATH} from "../../utils/Consts";
import {mapAdoptionData} from "../../adoption/model/AdoptionMapper";
import {generateUniqueDocumentId} from "../../utils/Helpers";
import {IDocumentFeedbackRepository} from "./IDocumentFeedbackRepository";

export class DocumentFeedbackRepositoryImpl implements IDocumentFeedbackRepository {
    ...
    async getAll(): Promise<Feedback[]> {
        return get(child(this.dbRef, DOCUMENT_FEEDBACK_PATH))
            .then((snapshot) => {
                if (snapshot.exists()) {
    ...
    async registerDocumentFeedback(feedBack: Feedback): Promise<boolean> {
        const documentId = generateUniqueDocumentId();

        feedBack["id"] = documentId

        const result = await set(ref(this.database,  DOCUMENT_FEEDBACK_PATH + '/' + documentId), feedBack)
            .then(() => true)
            .catch(() => false)
